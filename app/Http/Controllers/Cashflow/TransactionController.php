    ...
    class TransactionController extends Controller
    {
        ...
        public function store(TransactionRequest $request)
        {
            try {
                ...
                if ( $is_cash_in ) $account->increment( 'cash_in' , $amount );
                if ( $is_cash_out ) $account->increment( 'cash_out' , $amount + $data[ 'fee' ] );

                $this->saveMedia( $request , $transaction );
                $transaction->update( [ 'reference' => recordId( 'TX' , $transaction ) ] );
                activityLog( "Created Transaction {$transaction->reference}" , $transaction );
        ...
        public function update(TransactionRequest $request , Transaction $transaction)
        {
            try {
                ...
                if ( $transaction->cash_in > 0 ) $account->decrement( 'cash_in' , $transaction->cash_in );
                if ( $transaction->cash_out > 0 ) $account->decrement( 'cash_out' , $transaction->cash_out );
        ...
        public function destroy(Request $request)
        {
            try {
                ...
                foreach ( $ids as $id ) {
                    ...
                    activityLog( "Deleted Transaction {$transaction->reference}" , $transaction );
                    $transaction->delete();
                }
