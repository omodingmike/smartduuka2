    ...
    class ExpensesController extends Controller
    {
        ...
        public function store(ExpenseRequest $request)
        {
            try {
                ...
                $expenseType = $isRecurring == 1 ? 'Recurring' : 'Non-Recurring';
                $category    = $expense->expenseCategory?->name ?? NULL;
                $admins      = User::role( Role::ADMIN )->get();
                Notification::send( $admins , new ExpenseCreated(
                    title: 'New Expense Created' ,
                    message: "A new {$expenseType} expense has been recorded by " . auth()->user()->name . '.' ,
                    expenseId: $expense->expense_id ,
                    expenseName: $expense->name ,
                    amount: (float) $expense->amount ,
                    baseAmount: (float) $expense->base_amount ,
                    extraCharge: (float) $expense->extra_charge ,
        ...
        public function store1(ExpenseRequest $request)
        {
            try {
                $isRecurring = $request->integer( 'isRecurring' );
                $expense     = Expense::create( [
