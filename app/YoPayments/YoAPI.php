<?php
    ...
    class YoAPI
    {
        ...
        */
        ...
        protected function verify_payment_notification_old(Request $request) : bool
        {
            $data      = $request->date_time . $request->amount . $request->narrative . $request->network_ref . $request->external_ref .
                $request->msisdn;
            $signature = base64_decode( $request->signature );
            $fh        = fopen( app_path( 'YoPayments/public_key.pem' ) , 'r' );
            if ( $fh === FALSE ) {
                return FALSE;
            }
        ...
        protected function verify_payment_notification_official()
        {
