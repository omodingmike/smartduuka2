...
class DiningTableService
{
    ...
    public function store(DiningTableRequest $request)
    {
        try {
            ...
            $table =  DiningTable::create($request->validated() + ['qr_code' => 'storage/qr_codes/' . $filename, 'slug' => $slug]);
            activityLog('Created dining table: ' . $table->name);
            return $table;
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
    ...
    public function update(DiningTableRequest $request, DiningTable $diningTable)
    {
        try {
            ...
            $table = tap($diningTable)->update($request->validated() + ['qr_code' => 'storage/qr_codes/' . $filename, 'slug' => $slug]);
            activityLog('Updated dining table: ' . $table->name);
            return $table;
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
    ...
    public function destroy(DiningTable $diningTable): void
    {
        try {
            ...
            $diningTable->delete();
            activityLog('Deleted dining table: ' . $diningTable->name);
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
        }
