...
//                    $product->sellingUnits()->sync($request->other_unit_id);
        ...
        {
            try {
                $productVariation = ProductVariation::where( function ($query) use ($barcode) {
                    $query->where( 'sku' , $barcode )->orWhere( 'user_barcode' , $barcode );
                } )->first();
        ...
        public function generateSku($barcodeMethod)
        {
            try {
                // use ternary
                $length       = ( $barcodeMethod == 2 ) ? 12 : 13;
                $randomNumber = rand( pow( 10 , $length - 1 ) , pow( 10 , $length ) - 1 );
                return AppLibrary::sku( $randomNumber );
            } catch ( Exception $exception ) {
                Log::info( $exception->getMessage() );
                throw new Exception( $exception->getMessage() , 422 );
            }
...
//                                    }
        ...
        {
            try {
                return Product::select( [ 'id' , 'name' , 'buying_price' , 'can_purchasable' , 'status' , 'sku' , 'unit_id' , 'other_unit_id' , 'units_nature' ] )
                              ->with( 'unit:id,name,code' )
                              ->with( 'sellingUnits:id,name,code' )
                              ->with( 'prices' )
                              ->with( 'productTaxes' )
                              ->with( 'variations' )
